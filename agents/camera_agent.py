"""Camera Planning Agent — per-scene camera instructions."""
...
from core.errors import CameraAgentError, StorageError
from core.logging import get_logger
from core.paths import (
    ensure_project_analysis_dir,
    ensure_project_dir,
    get_camera_plan_path,
)
from schemas.camera import CameraPack, CameraResult
from schemas.job import FeatureFlags, VideoJobConfig
from schemas.project import ProjectMetadata
from tools.camera.catalog import AnalyzeFn, build_camera_pack

logger = get_logger(__name__)


class CameraAgent(BaseAgent):
    """Write analysis/camera_plan.json (plan-only camera instructions)."""
    ...
    def run(
        ...
        **_: Any,
        ...
        try:
            ...
            raise CameraAgentError(f"Camera planning failed: {exc}") from exc

        path = self._write_pack(project_id, root, pack)
        public = CameraResult(
            camera_pack=pack, camera_path=str(path)
        ).public_output()
        messages = [
            f"[{self.name}] {pack.notes}",
            f"[{self.name}] Scenes: {len(pack.plan.instructions)}",
            f"[{self.name}] Wrote analysis/camera_plan.json",
        ]
        if public.get("instructions"):
            messages.append(
                f"[{self.name}] Sample: "
                f"{json.dumps(public['instructions'][0], ensure_ascii=False)[:160]}"
            )
        logger.info(
            "CameraAgent ready project_id=%s scenes=%s skipped=%s",
            project_id,
            len(pack.plan.instructions),
            pack.plan.skipped,
        )
        return CameraResult(
            camera_pack=pack, camera_path=str(path), messages=messages
        )
    ...
    def _coerce_project(
        ...
        try:
            ...
            raise CameraAgentError(f"Invalid project metadata: {exc}") from exc
    ...
    def _coerce_config(
        ...
        try:
            ...
            raise CameraAgentError(f"Invalid job config: {exc}") from exc
    ...
    def _coerce_features(
        ...
        try:
            ...
            raise CameraAgentError(f"Invalid feature flags: {exc}") from exc
    ...
    def _write_pack(
        ...
        path = root / "analysis" / "camera_plan.json"
        try:
            try:
                canonical = get_camera_plan_path(project_id)
            ...
            raise StorageError(f"Failed to write camera_plan.json: {path}") from exc
        return path.resolve()
